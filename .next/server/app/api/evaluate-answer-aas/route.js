...
총점: 3
총체적_채점: B(중) 수준
피드백: 엔지니어가 되고 싶은 이유를 잘 표현했어요. 철자를 조금 더 정확하게 쓰고, 더 구체적인 이유도 추가해 보세요.`}]}getPromptTemplate(){let e=this.getExamples(),r=new l.PromptTemplate({inputVariables:["criteria","request","answer"],template:"criteria: {criteria} \n request: {request} \n answer: {answer}"});return new l.FewShotPromptTemplate({examples:e,examplePrompt:r,prefix:`당신은 초등 영어 서술형 평가 문항의 전문 채점자이며, 주어진 채점 기준에 따라 **정확하고 일관된 평가**를 수행해야 합니다.
...
아래는 채점 예시입니다.:`,suffix:"criteria: {criteria} \n request: {request} \n answer:",inputVariables:["criteria","request"]})}async retrieve(e){this.retrieval||await this.initializeRetrieval();let r=await this.retrieval.invoke(e.request);return e.criteria=r.map(e=>e.pageContent).join("\n\n"),e}async generate(e){let r=this.getPromptTemplate().pipe(this.llm).pipe(new c.StringOutputParser),a=await r.invoke({criteria:e.criteria,request:e.request});try{let r=a.replace(/```json\n?/g,"").replace(/```\n?/g,"").trim(),t=JSON.parse(r);e.answer=p.parse(t)}catch(e){throw Error("생성된 답안을 파싱할 수 없습니다.")}return e}async criteriaAnswerRelevanceJudge(e){let r=l.ChatPromptTemplate.fromMessages([["system",`당신은 초등 영어 서술형 평가 자동 채점 결과의 정확성을 평가하는 전문가입니다.
...
점수는 숫자만 반환하세요.`],["human","채점 기준: {criteria} \n LLM 채점 결과: {answer}"]]),a=e.answer?JSON.stringify(e.answer,null,2):"",t=r.pipe(this.llm).pipe(new c.StringOutputParser),i=await t.invoke({criteria:e.criteria,answer:a});return parseInt(i.match(/\d+/)?.[0]||"1")}async feedbackQualityJudge(e){let r=l.ChatPromptTemplate.fromMessages([["system",`당신은 초등 영어 서술형 평가의 피드백 품질을 평가하는 전문가입니다.
...
점수는 숫자만 반환하세요.`],["human","채점 및 피드백 생성 결과: {answer}"]]),a=e.answer?JSON.stringify(e.answer,null,2):"",t=r.pipe(this.llm).pipe(new c.StringOutputParser),i=await t.invoke({answer:a});return parseInt(i.match(/\d+/)?.[0]||"1")}async requestRewriterHighHallucination(e){let r=l.ChatPromptTemplate.fromMessages([["system",`LLM이 생성한 채점 결과가 채점 기준이나 교사 채점 예시와 일치하지 않아 채점 정확도와 신뢰도가 낮게 평가되었습니다.
...
수정된 요청은 채점 결과의 정확성과 신뢰도를 높이기 위한 것입니다.`],["human","요청: {request}"]]).pipe(this.llm).pipe(new c.StringOutputParser);return e.request=await r.invoke({request:e.request}),e}async requestRewriterLowFeedback(e){let r=l.ChatPromptTemplate.fromMessages([["system",`LLM이 생성한 피드백이 다음 기준에 미흡하여 낮은 점수를 받았습니다:
