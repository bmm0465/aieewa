...
요청은 생성 결과가 문서에 충실하게 기초할 수 있도록 유도해야 합니다.`],["human","요청: {request}"]]).pipe(this.llmGen).pipe(new u.StringOutputParser);return e.request=await t.invoke({request:e.request}),e}async run(e){try{console.log("AQG Agent run 시작:",e),await this.initializeRetrieval(),console.log("Retrieval 초기화 완료");let t={request:e,context:""},r=0;for(;r<2;)try{console.log(`AQG 워크플로 시도 ${r+1}/2`),t=await this.retrieve(t),console.log("검색 단계 완료");try{let e=await this.requestContextRelevanceJudge(t);if(console.log("관련성 판단:",e),"not relevant"===e){t=await this.requestRewriterLowContext(t);continue}}catch(e){console.warn("관련성 판단 오류, 계속 진행:",e)}if(t=await this.generate(t),console.log("문항 생성 완료:",!!t.answer),t.answer)try{let e=await this.contextAnswerRelevanceJudge(t);if(console.log("환각 판단:",e),"finish"===e)break}catch(e){console.warn("환각 판단 오류, 생성된 결과 사용:",e);break}r++}catch(e){if(console.error(`AQG 워크플로 단계 ${r+1} 오류:`,e),++r>=2)try{if(console.log("간단한 문항 생성 시도"),(t=await this.generate(t)).answer){console.log("간단한 생성 성공");break}}catch(e){console.error("간단한 생성도 실패:",e)}}if(console.log("AQG 워크플로 완료, 결과:",!!t.answer),!t.answer)throw Error("문항 생성을 완료할 수 없습니다. 요청을 다시 확인해주세요.");return t.answer}catch(e){throw console.error("AQG Agent 전체 실행 오류:",e),Error(`문항 생성 중 오류가 발생했습니다: ${e instanceof Error?e.message:"알 수 없는 오류"}`)}}}let y="nodejs",f=60,E="force-dynamic";async function S(e){try{let t,r;console.log("문항 생성 요청 시작");let o=await e.json();console.log("요청 본문:",o);let{request:n,selectedGrade:s,selectedLesson:a}=o;if(!n)return console.error("요청 파라미터 누락:",o),c.Z.json({error:"요청(request)이 필요합니다."},{status:400});let l=n;if(s&&a&&(console.log("단원 정보 추가:",{selectedGrade:s,selectedLesson:a}),l=`${n}

[단원 정보]
- 학년: ${s}
- 단원: ${a}

이 단원의 학습 목표와 핵심 내용을 고려하여 적절한 난이도의 서술형 평가 문항을 생성해주세요.`),console.log("환경 변수 확인 중..."),console.log("OPENAI_API_KEY 존재:",!!process.env.OPENAI_API_KEY),console.log("OPENAI_API_KEY 길이:",process.env.OPENAI_API_KEY?.length||0),!process.env.OPENAI_API_KEY)return console.error("OPENAI_API_KEY 환경 변수가 설정되지 않았습니다."),c.Z.json({error:"OpenAI API 키가 설정되지 않았습니다. 관리자에게 문의해주세요."},{status:500});let i="https://puajxghokhribumtxrby.supabase.co",u=process.env.SUPABASE_SERVICE_ROLE_KEY;console.log("Supabase 환경 변수 상태:",{NEXT_PUBLIC_SUPABASE_URL:!!i,SUPABASE_SERVICE_ROLE_KEY:!!u}),i&&u||console.warn("Supabase 환경 변수가 일부 누락되었습니다:",{NEXT_PUBLIC_SUPABASE_URL:!!i,SUPABASE_SERVICE_ROLE_KEY:!!u}),console.log("AQG 에이전트 초기화 시작");try{t=new w,console.log("AQG 에이전트 초기화 완료")}catch(e){return console.error("AQG 에이전트 초기화 실패:",e),c.Z.json({error:`AI 모델 초기화에 실패했습니다: ${e instanceof Error?e.message:"알 수 없는 오류"}`},{status:500})}console.log("AQG 에이전트 run 메서드 호출");try{r=await t.run(l),console.log("AQG 에이전트 실행 완료:",!!r)}catch(t){console.error("AQG 에이전트 실행 실패:",t);let e="AI가 문항 생성 중 오류가 발생했습니다.";return t instanceof Error&&(e=t.message,t.message.includes("API key")||t.message.includes("인증")?e="AI 서비스 인증에 문제가 있습니다.":t.message.includes("rate limit")||t.message.includes("사용량")?e="AI 서비스 사용량이 초과되었습니다. 잠시 후 다시 시도해주세요.":t.message.includes("network")||t.message.includes("네트워크")?e="AI 서비스 연결에 문제가 있습니다.":(t.message.includes("파싱")||t.message.includes("parsing"))&&(e="AI 응답 처리 중 오류가 발생했습니다. 다시 시도해주세요.")),c.Z.json({error:e},{status:500})}if(!r)return console.error("AQG 에이전트가 빈 결과를 반환했습니다."),c.Z.json({error:"AI가 문항을 생성하지 못했습니다."},{status:500});let _=["단원_및_학년","예시문","평가문항","조건"].filter(e=>!r[e]);if(_.length>0)return console.error("필수 필드 누락:",_,r),c.Z.json({error:`생성된 문항에 필수 정보가 누락되었습니다: ${_.join(", ")}`},{status:500});console.log("데이터베이스 저장 시작");try{if(!i||!u)return console.warn("Supabase 환경 변수가 누락되어 데이터베이스 저장을 건너뜁니다."),c.Z.json({success:!0,question:r,questionId:null,warning:"문항은 생성되었지만 데이터베이스 설정이 누락되어 저장되지 않았습니다."});let e=(0,d.B$)();console.log("Supabase 클라이언트 생성 완료");let{data:t,error:o}=await e.from("questions").insert({unit_grade:r.단원_및_학년,example_text:r.예시문,question:r.평가문항,conditions:r.조건,model_answer_1:r.모범_답안_1||"",model_answer_2:r.모범_답안_2||"",analytical_criteria_1:r.분석적_채점_기준_1||"",analytical_criteria_2:r.분석적_채점_기준_2||"",analytical_criteria_3:r.분석적_채점_기준_3||"",holistic_criteria_a:r.총체적_채점_기준_A||"",holistic_criteria_b:r.총체적_채점_기준_B||"",holistic_criteria_c:r.총체적_채점_기준_C||"",example_answer_a:r.성취수준별_예시_답안_A||"",example_answer_b:r.성취수준별_예시_답안_B||"",example_answer_c:r.성취수준별_예시_답안_C||"",feedback_a:r.성취수준별_평가에_따른_예시_피드백_A||"",feedback_b:r.성취수준별_평가에_따른_예시_피드백_B||"",feedback_c:r.성취수준별_평가에_따른_예시_피드백_C||""}).select().single();if(o)return console.error("문항 저장 오류:",o),c.Z.json({error:`문항 저장에 실패했습니다: ${o.message}`},{status:500});console.log("문항 저장 성공:",t);try{await e.from("evaluation_sessions").insert({request:n,generated_question_id:t.id,status:"generated"}),console.log("평가 세션 저장 완료")}catch(e){console.warn("평가 세션 저장 실패 (무시됨):",e)}return c.Z.json({success:!0,question:r,questionId:t.id})}catch(e){return console.error("데이터베이스 연동 오류:",e),c.Z.json({success:!0,question:r,questionId:null,warning:"문항은 생성되었지만 데이터베이스 저장에 실패했습니다."})}}catch(t){console.error("문항 생성 전체 오류:",t);let e="문항 생성에 실패했습니다.";return t instanceof Error&&(e=t.message,t.message.includes("API key")?e="OpenAI API 키 설정에 문제가 있습니다.":t.message.includes("network")||t.message.includes("fetch")?e="네트워크 연결에 문제가 있습니다.":t.message.includes("timeout")&&(e="요청 시간이 초과되었습니다. 다시 시도해주세요.")),c.Z.json({error:e,details:void 0},{status:500})}}let b=new n.AppRouteRouteModule({definition:{kind:s.x.APP_ROUTE,page:"/api/generate-question/route",pathname:"/api/generate-question",filename:"route",bundlePath:"app/api/generate-question/route"},resolvedPagePath:"C:\\Users\\김민제\\Documents\\Dev\\aieewa\\app\\api\\generate-question\\route.ts",nextConfigOutput:"",userland:o}),{requestAsyncStorage:v,staticGenerationAsyncStorage:x,serverHooks:P,headerHooks:I,staticGenerationBailout:C}=b,q="/api/generate-question/route";function R(){return(0,a.patchFetch)({serverHooks:P,staticGenerationAsyncStorage:x})}},7399:(e,t,r)=>{r.d(t,{Hy:()=>u,eB:()=>l,yD:()=>i});var o=r(8775),n=r(8234);let s=null;function a(){if(!s){if(!process.env.OPENAI_API_KEY)throw Error("OPENAI_API_KEY 환경 변수가 설정되지 않았습니다.");s=new o.OpenAIEmbeddings({openAIApiKey:process.env.OPENAI_API_KEY,modelName:"text-embedding-3-small",batchSize:100})}return s}async function c(e){try{console.log(`${e.length}개 텍스트에 대한 임베딩 생성 시작`);let t=a(),r=await t.embedDocuments(e);return console.log(`임베딩 생성 완료: ${r.length}개 벡터`),r}catch(e){throw console.error("임베딩 생성 오류:",e),Error(`임베딩 생성에 실패했습니다: ${e instanceof Error?e.message:"알 수 없는 오류"}`)}}async function l(e){try{let t=a();return await t.embedQuery(e)}catch(e){throw console.error("단일 임베딩 생성 오류:",e),Error(`임베딩 생성에 실패했습니다: ${e instanceof Error?e.message:"알 수 없는 오류"}`)}}async function i(e,t=10,r=.7){try{let o=(0,n.B$)(),{data:s,error:a}=await o.from("document_chunks").select("chunk_text, metadata, embedding").not("embedding","is",null);if(a)return console.error("청크 조회 오류:",a),[];if(!s||0===s.length)return console.log("임베딩된 청크가 없습니다."),[];let c=s.map(t=>{if(!t.embedding||!Array.isArray(t.embedding))return{...t,similarity:0};let r=function(e,t){if(e.length!==t.length)throw Error("벡터 차원이 일치하지 않습니다.");let r=0,o=0,n=0;for(let s=0;s<e.length;s++)r+=e[s]*t[s],o+=e[s]*e[s],n+=t[s]*t[s];return 0===o||0===n?0:r/(Math.sqrt(o)*Math.sqrt(n))}(e,t.embedding);return{...t,similarity:r}}).filter(e=>e.similarity>=r).sort((e,t)=>t.similarity-e.similarity).slice(0,t);return console.log(`벡터 검색 결과: ${c.length}개 (임계값: ${r})`),c}catch(e){return console.error("벡터 검색 오류:",e),[]}}async function u(e,t){try{let r=(0,n.B$)(),o=Math.ceil(e.length/50);console.log(`${e.length}개 청크를 ${o}개 배치로 처리`);for(let n=0;n<e.length;n+=50){let s=e.slice(n,n+50),a=Math.floor(n/50)+1;console.log(`배치 ${a}/${o} 처리 중... (${s.length}개 청크)`);try{let e=s.map(e=>e.text),n=await c(e),l=s.map((e,r)=>({document_id:t,chunk_text:e.text,chunk_index:e.chunkIndex,metadata:e.metadata,embedding:n[r]})),{error:i}=await r.from("document_chunks").insert(l);i?console.error(`배치 ${a} 저장 오류:`,i):console.log(`배치 ${a} 저장 완료`),a<o&&await new Promise(e=>setTimeout(e,1e3))}catch(e){console.error(`배치 ${a} 처리 오류:`,e)}}console.log("모든 청크 임베딩 처리 완료")}catch(e){throw console.error("청크 임베딩 처리 오류:",e),e}}},8234:(e,t,r)=>{r.d(t,{B$:()=>n});var o=r(7385);function n(){let e="https://puajxghokhribumtxrby.supabase.co",t=process.env.SUPABASE_SERVICE_ROLE_KEY;if(!e||!t){let r=[];throw e||r.push("NEXT_PUBLIC_SUPABASE_URL"),t||r.push("SUPABASE_SERVICE_ROLE_KEY"),Error(`Missing Supabase environment variables: ${r.join(", ")}`)}return(0,o.eI)(e,t)}}};var t=require("../../../webpack-runtime.js");t.C(e);var r=e=>t(t.s=e),o=t.X(0,[638,117,256],()=>r(2638));module.exports=o})();