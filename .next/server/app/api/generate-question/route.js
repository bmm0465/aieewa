...
성취수준별 평가에 따른 예시 피드백 C: 자신이 계획한 행사가 어떤 것인지에 대한 정보가 부족합니다. 초대하는 글에 들어가는 행사 이름, 날짜와 시간, 장소, 할 일에 대한 어휘를 학습하고 문장으로 쓰는 연습을 해 보세요. 그림과 함께 있는 쉽고 간단한 초대 글을 찾아 읽기를 추천합니다.`}],t=new u.PromptTemplate({inputVariables:["context","request","answer"],template:"context: {context} \n request: {request} \n answer: {answer}"});return new u.FewShotPromptTemplate({examples:e,examplePrompt:t,prefix:`당신은 초등 영어 서술형 평가 문항 및 채점 기준을 전문적으로 개발하는 평가 전문가입니다. 
...
아래는 평가 문항과 채점 기준 예시입니다.:`,suffix:"context: {context} \n request: {request} \n answer:",inputVariables:["context","request"]})}async retrieve(e){this.retrieval||await this.initializeRetrieval();let t=await this.retrieval.invoke(e.request);return e.context=t.map(e=>e.pageContent).join("\n\n"),e}async requestContextRelevanceJudge(e){let t=u.ChatPromptTemplate.fromMessages([["system",`당신은 초등 영어 서술형 평가 문항과 채점 기준 개발 전문가입니다.
...
점수는 숫자로만 반환하세요.`],["human","요청: {request} \n 검색된 문서: {context}"]]).pipe(this.llmJudge).pipe(new c.StringOutputParser),r=await t.invoke({request:e.request,context:e.context});return parseInt(r.match(/\d+/)?.[0]||"1")>=3?"relevant":"not relevant"}async generate(e){let t=this.getPromptTemplate().pipe(this.llmGen).pipe(new c.StringOutputParser),r=await t.invoke({context:e.context,request:e.request});try{let t=r.replace(/```json\n?/g,"").replace(/```\n?/g,"").trim(),a=JSON.parse(t);e.answer=p.parse(a)}catch(e){throw Error("생성된 답안을 파싱할 수 없습니다.")}return e}async contextAnswerRelevanceJudge(e){let t=u.ChatPromptTemplate.fromMessages([["system",`당신은 초등 영어 서술형 평가 문항과 채점 기준 개발 전문가입니다. 
...
점수는 숫자로만 반환하세요.`],["human","검색된 문서: {context} \n 생성된 평가 문항과 채점 기준: {answer}"]]),r=e.answer?JSON.stringify(e.answer,null,2):"",a=t.pipe(this.llmJudge).pipe(new c.StringOutputParser),s=await a.invoke({context:e.context,answer:r});return parseInt(s.match(/\d+/)?.[0]||"1")>=3?"finish":"hallucination"}async requestRewriterLowContext(e){let t=u.ChatPromptTemplate.fromMessages([["system",`검색된 문서와 요청의 관련성이 낮습니다. 요청을 벡터 검색 시스템이 더 정확하게 문서를 검색할 수 있도록 다음과 같이 바꿔주세요:
- 요청의 핵심 주제와 목적을 명확하게 표현
- 불필요하거나 모호한 표현 제거
- 단원, 학년, 주제 키워드 강조
- 예시문 내용과 관련된 핵심 개념이 드러나도록

재작성된 요청은 문서 검색 정확도를 높이는 것을 목표로 합니다.`],["human","요청: {request}"]]).pipe(this.llmGen).pipe(new c.StringOutputParser);return e.request=await t.invoke({request:e.request}),e}async requestRewriterHighHallucination(e){let t=u.ChatPromptTemplate.fromMessages([["system",`생성된 평가 문항과 채점 기준이 검색된 문서에 제대로 기반하지 않았습니다. 
이 문제를 해결하기 위해, 요청을 다음과 같은 방향으로 바꿔주세요:
- 요청 목적을 더 구체적으로 설명
- 참고할 문서의 주제나 내용 키워드를 명확히 제시
- 예시문, 단원, 학년 등에서 반드시 포함되어야 하는 핵심 정보 포함

요청은 생성 결과가 문서에 충실하게 기초할 수 있도록 유도해야 합니다.`],["human","요청: {request}"]]).pipe(this.llmGen).pipe(new c.StringOutputParser);return e.request=await t.invoke({request:e.request}),e}async run(e){await this.initializeRetrieval();let t={request:e,context:""},r=0;for(;r<3;)try{t=await this.retrieve(t);let e=await this.requestContextRelevanceJudge(t);if("not relevant"===e){t=await this.requestRewriterLowContext(t);continue}t=await this.generate(t);let a=await this.contextAnswerRelevanceJudge(t);if("finish"===a)break;t=await this.requestRewriterHighHallucination(t),r++}catch(e){r++}if(!t.answer)throw Error("문항 생성을 완료할 수 없습니다.");return t.answer}}var h=r(8234);let d="nodejs",w=60,v="force-dynamic";async function x(e){try{let{request:t}=await e.json();if(!t)return i.Z.json({error:"요청(request)이 필요합니다."},{status:400});let r=new m,a=await r.run(t),s=(0,h.B$)(),{data:n,error:o}=await s.from("questions").insert({unit_grade:a.단원_및_학년,example_text:a.예시문,question:a.평가문항,conditions:a.조건,model_answer_1:a.모범_답안_1,model_answer_2:a.모범_답안_2,analytical_criteria_1:a.분석적_채점_기준_1,analytical_criteria_2:a.분석적_채점_기준_2,analytical_criteria_3:a.분석적_채점_기준_3,holistic_criteria_a:a.총체적_채점_기준_A,holistic_criteria_b:a.총체적_채점_기준_B,holistic_criteria_c:a.총체적_채점_기준_C,example_answer_a:a.성취수준별_예시_답안_A,example_answer_b:a.성취수준별_예시_답안_B,example_answer_c:a.성취수준별_예시_답안_C,feedback_a:a.성취수준별_평가에_따른_예시_피드백_A,feedback_b:a.성취수준별_평가에_따른_예시_피드백_B,feedback_c:a.성취수준별_평가에_따른_예시_피드백_C}).select().single();if(o)return i.Z.json({error:"문항 저장에 실패했습니다."},{status:500});return await s.from("evaluation_sessions").insert({request:t,generated_question_id:n.id,status:"generated"}),i.Z.json({success:!0,question:a,questionId:n.id})}catch(e){return i.Z.json({error:"문항 생성에 실패했습니다."},{status:500})}}let q=new s.AppRouteRouteModule({definition:{kind:n.x.APP_ROUTE,page:"/api/generate-question/route",pathname:"/api/generate-question",filename:"route",bundlePath:"app/api/generate-question/route"},resolvedPagePath:"C:\\Users\\bmm04\\Dev\\aieewa\\app\\api\\generate-question\\route.ts",nextConfigOutput:"",userland:a}),{requestAsyncStorage:b,staticGenerationAsyncStorage:g,serverHooks:y,headerHooks:f,staticGenerationBailout:A}=q,C="/api/generate-question/route";function P(){return(0,o.patchFetch)({serverHooks:y,staticGenerationAsyncStorage:g})}},8234:(e,t,r)=>{r.d(t,{B$:()=>s});var a=r(7385);function s(){let e=process.env.NEXT_PUBLIC_SUPABASE_URL,t=process.env.SUPABASE_SERVICE_ROLE_KEY;if(!e||!t)throw Error("Missing Supabase environment variables");return(0,a.eI)(e,t)}}};var t=require("../../../webpack-runtime.js");t.C(e);var r=e=>t(t.s=e),a=t.X(0,[638,117,256],()=>r(9901));module.exports=a})();