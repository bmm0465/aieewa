...
요청은 생성 결과가 문서에 충실하게 기초할 수 있도록 유도해야 합니다.`],["human","요청: {request}"]]).pipe(this.llmGen).pipe(new _.StringOutputParser);return e.request=await t.invoke({request:e.request}),e}async run(e){try{console.log("AQG Agent run 시작:",e),await this.initializeRetrieval(),console.log("Retrieval 초기화 완료");let t={request:e,context:""},r=0;for(;r<2;)try{console.log(`AQG 워크플로 시도 ${r+1}/2`),t=await this.retrieve(t),console.log("검색 단계 완료");try{let e=await this.requestContextRelevanceJudge(t);if(console.log("관련성 판단:",e),"not relevant"===e){t=await this.requestRewriterLowContext(t);continue}}catch(e){console.warn("관련성 판단 오류, 계속 진행:",e)}if(t=await this.generate(t),console.log("문항 생성 완료:",!!t.answer),t.answer)try{let e=await this.contextAnswerRelevanceJudge(t);if(console.log("환각 판단:",e),"finish"===e)break}catch(e){console.warn("환각 판단 오류, 생성된 결과 사용:",e);break}r++}catch(e){if(console.error(`AQG 워크플로 단계 ${r+1} 오류:`,e),++r>=2)try{if(console.log("간단한 문항 생성 시도"),(t=await this.generate(t)).answer){console.log("간단한 생성 성공");break}}catch(e){console.error("간단한 생성도 실패:",e)}}if(console.log("AQG 워크플로 완료, 결과:",!!t.answer),!t.answer)throw Error("문항 생성을 완료할 수 없습니다. 요청을 다시 확인해주세요.");return t.answer}catch(e){throw console.error("AQG Agent 전체 실행 오류:",e),Error(`문항 생성 중 오류가 발생했습니다: ${e instanceof Error?e.message:"알 수 없는 오류"}`)}}}var h=r(8234);let g="nodejs",d=60,A="force-dynamic";async function w(e){try{console.log("문항 생성 요청 시작");let t=await e.json();console.log("요청 본문:",t);let{request:r}=t;if(!r)return console.error("요청 파라미터 누락:",t),i.Z.json({error:"요청(request)이 필요합니다."},{status:400});if(!process.env.OPENAI_API_KEY)return console.error("OPENAI_API_KEY 환경 변수가 설정되지 않았습니다."),i.Z.json({error:"OpenAI API 키가 설정되지 않았습니다."},{status:500});console.log("AQG 에이전트 초기화 시작");let o=new m;console.log("AQG 에이전트 run 메서드 호출");let s=await o.run(r);if(console.log("AQG 에이전트 실행 완료:",s),!s)return console.error("AQG 에이전트가 빈 결과를 반환했습니다."),i.Z.json({error:"AI가 문항을 생성하지 못했습니다."},{status:500});let n=["단원_및_학년","예시문","평가문항","조건"].filter(e=>!s[e]);if(n.length>0)return console.error("필수 필드 누락:",n,s),i.Z.json({error:`생성된 문항에 필수 정보가 누락되었습니다: ${n.join(", ")}`},{status:500});console.log("데이터베이스 저장 시작");try{let e=(0,h.B$)();console.log("Supabase 클라이언트 생성 완료");let{data:t,error:o}=await e.from("questions").insert({unit_grade:s.단원_및_학년,example_text:s.예시문,question:s.평가문항,conditions:s.조건,model_answer_1:s.모범_답안_1||"",model_answer_2:s.모범_답안_2||"",analytical_criteria_1:s.분석적_채점_기준_1||"",analytical_criteria_2:s.분석적_채점_기준_2||"",analytical_criteria_3:s.분석적_채점_기준_3||"",holistic_criteria_a:s.총체적_채점_기준_A||"",holistic_criteria_b:s.총체적_채점_기준_B||"",holistic_criteria_c:s.총체적_채점_기준_C||"",example_answer_a:s.성취수준별_예시_답안_A||"",example_answer_b:s.성취수준별_예시_답안_B||"",example_answer_c:s.성취수준별_예시_답안_C||"",feedback_a:s.성취수준별_평가에_따른_예시_피드백_A||"",feedback_b:s.성취수준별_평가에_따른_예시_피드백_B||"",feedback_c:s.성취수준별_평가에_따른_예시_피드백_C||""}).select().single();if(o)return console.error("문항 저장 오류:",o),i.Z.json({error:`문항 저장에 실패했습니다: ${o.message}`},{status:500});console.log("문항 저장 성공:",t);try{await e.from("evaluation_sessions").insert({request:r,generated_question_id:t.id,status:"generated"}),console.log("평가 세션 저장 완료")}catch(e){console.warn("평가 세션 저장 실패 (무시됨):",e)}return i.Z.json({success:!0,question:s,questionId:t.id})}catch(e){return console.error("데이터베이스 연동 오류:",e),i.Z.json({success:!0,question:s,questionId:null,warning:"문항은 생성되었지만 데이터베이스 저장에 실패했습니다."})}}catch(t){console.error("문항 생성 전체 오류:",t);let e="문항 생성에 실패했습니다.";return t instanceof Error&&(e=t.message,t.message.includes("API key")?e="OpenAI API 키 설정에 문제가 있습니다.":t.message.includes("network")||t.message.includes("fetch")?e="네트워크 연결에 문제가 있습니다.":t.message.includes("timeout")&&(e="요청 시간이 초과되었습니다. 다시 시도해주세요.")),i.Z.json({error:e,details:void 0},{status:500})}}let v=new s.AppRouteRouteModule({definition:{kind:n.x.APP_ROUTE,page:"/api/generate-question/route",pathname:"/api/generate-question",filename:"route",bundlePath:"app/api/generate-question/route"},resolvedPagePath:"C:\\Users\\bmm04\\Dev\\aieewa\\app\\api\\generate-question\\route.ts",nextConfigOutput:"",userland:o}),{requestAsyncStorage:x,staticGenerationAsyncStorage:f,serverHooks:q,headerHooks:y,staticGenerationBailout:b}=v,P="/api/generate-question/route";function C(){return(0,a.patchFetch)({serverHooks:q,staticGenerationAsyncStorage:f})}},8234:(e,t,r)=>{r.d(t,{B$:()=>s});var o=r(7385);function s(){let e=process.env.NEXT_PUBLIC_SUPABASE_URL,t=process.env.SUPABASE_SERVICE_ROLE_KEY;if(!e||!t)throw Error("Missing Supabase environment variables");return(0,o.eI)(e,t)}}};var t=require("../../../webpack-runtime.js");t.C(e);var r=e=>t(t.s=e),o=t.X(0,[638,117,256],()=>r(9901));module.exports=o})();